...
async def process_time_selection(callback: CallbackQuery, state: FSMContext):
    if callback.data == "time_custom":
        ...
        response = await callback.message.answer("Введите время в формате ЧЧ:ММ (например, 14:30):")
        ...
        await message_cleaner.delete_previous_messages(callback.bot, callback.from_user.id)
        response = await callback.message.answer("Укажите максимальное количество игроков:",
